...
interface Props {
  ...
  onChange: (index: number, field: keyof ContentItem, value: string) => void;
  onAdd: () => void;
  onRemove: (index: number) => void;
  onExpand: (index: number) => void;
  onSubmit: (e: React.FormEvent) => void;
  onBack: () => void;
  onProfileRemoval?: () => void;
}

export default function ContentFormGoogleClean({
  contentItems,
  expandedIndex,
  totalPrice,
  businessCardName,
  ...
  // Auto-fill the URL field with the business card name when expanding a content item
  const handleExpand = (index: number) => {
    onExpand(index);
    
    // If expanding and the URL is empty, auto-fill with business card name
    if (expandedIndex !== index && !contentItems[index].url) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      {/* Komunikat o wyłączonych treściach */}
      {!contentEnabled && (
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-orange-800">
                Zbyt duża ilość zgłoszeń na usuwanie treści
              </h3>
              <div className="mt-2 text-sm text-orange-700">
                <p>Z powodu dużej ilości zgłoszeń obecnie nie przyjmujemy nowych zleceń na usuwanie pojedynczych treści.</p>
                <p className="mt-2 font-medium">Proponujemy usunięcie całej Wizytówki Google lub zresetowanie wszystkich treści.</p>
      ...
      {/* Lista treści - wyblurowana gdy wyłączone */}
      <div className={!contentEnabled ? 'blur-sm pointer-events-none opacity-50' : ''}>
          ...
          >
            <div className="flex justify-between items-center">
              <div className="text-sm text-gray-700">
                <p className="font-medium">
                  Opinia #{index + 1} {item.author && `– ${item.author}`}
                </p>
            ...
            {expandedIndex === index && (
              <>
                ...
                <input
                  type="date"
                  placeholder="Data dodania treści"
      ...
      <p className="text-sm text-center text-gray-700">
        Dodaj autora i datę w celu dokładnej weryfikacji.
      </p>
      <p className="text-sm text-center text-gray-700">
